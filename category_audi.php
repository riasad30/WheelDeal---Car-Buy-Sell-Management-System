<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Audi Category</title>
    <link rel="stylesheet" href="css/style_user.css" />
    <link rel="icon" href="images/favicon.png" />
</head>

<section class="navbar">
    <div class="container">
        <div class="logo">
            
            <a href="client/user_page.php"><img src="images/logo.png" alt="Car management Logo"
                    class="img-responsive" /></a>
        </div>
        <div class="menu text-right">
            
            <ul type="none">
                <li><a href="client/user_page.php">Home</a></li>
                <li><a href="categories.php">Categories</a></li>
                <li><a href="cars.php">Cars</a></li>
                <li><a href="community.php">Community</a></li>
                <li><a href="logout.php">Logout</a></li>
            </ul>
        </div>
        <div class="clearfix"></div>
    </div>
</section>

<section class="car-search text-center">
    <br>
    <br>
    <br>
</section>


<section class="car-menu">
    <div class="container">
        <h2 class="text-center">Cars on <a href="#" class="text-orange text-center">"Audi"</a> Category</h2>
       
        <div class="car-menu-box">
            <div class="car-menu-img">
                <img src="images/audi1.png" alt="" class="img-curve img-responsive">
            </div>
            <div class="car-menu-desc">
                <h4>Car Title</h4>
                <p class="car-status">Location</p>
                <p class="car-desc">condition is perfect</p>
                <br>
                <a href="buy.php" class="btn btn-primary">Buy</a>
               
            </div>
        </div>

        <div class="car-menu-box">
            <div class="car-menu-img">
                <img src="images/audi2.png" alt="" class="img-curve img-responsive">
            </div>
            <div class="car-menu-desc">
                <h4>Car Title</h4>
                <p class="car-status">Location</p>
                <p class="car-desc">condition is perfect</p>
                <br>
                <a href="buy.php" class="btn btn-primary">Buy</a>

            </div>
        </div>
        <div class="car-menu-box">
            <div class="car-menu-img">
                <img src="images/audi3.png" alt="" class="img-curve img-responsive">
            </div>
            <div class="car-menu-desc">
                <h4>Car Title</h4>
                <p class="car-status">Location</p>
                <p class="car-desc">condition is perfect</p>
                <br>
                <a href="buy.php" class="btn btn-primary">Buy</a>

            </div>
        </div>
        <div class="car-menu-box">
            <div class="car-menu-img">
                <img src="images/audi4.png" alt="" class="img-curve img-responsive">
            </div>
            <div class="car-menu-desc">
                <h4>Car Title</h4>
                <p class="car-status">Location</p>
                <p class="car-desc">condition is perfect</p>
                <br>
                <a href="buy.php" class="btn btn-primary">Buy</a>

            </div>
        </div>
        <div class="car-menu-box">
            <div class="car-menu-img">
                <img src="images/audi4.png" alt="" class="img-curve img-responsive">
            </div>
            <div class="car-menu-desc">
                <h4>Car Title</h4>
                <p class="car-status">Location</p>
                <p class="car-desc">condition is perfect</p>
                <br>
                <a href="buy.php" class="btn btn-primary">Buy</a>

            </div>
        </div>
        <div class="car-menu-box">
            <div class="car-menu-img">
                <img src="images/audi5.png" alt="" class="img-curve img-responsive">
            </div>
            <div class="car-menu-desc">
                <h4>Car Title</h4>
                <p class="car-status">Location</p>
                <p class="car-desc">condition is perfect</p>
                <br>
                <a href="buy.php" class="btn btn-primary">Buy</a>
                
            </div>
        </div>
        <div class="clearfix"></div>
    </div>
</section>
<div class="mp-21"></div>

<section class="social">
    <div class="container text-center">
        <ul type="none">
            <li>
                <a href="#"><img src="images/facebook.png" alt="facebook" class="social-img img-responsive"></a>
            </li>
            <li>
                <a href="#"><img src="images/instagram.png" alt="instagram" class="social-img img-responsive"></a>
            </li>
            <li>
                <a href="#"><img src="images/twitter.png" alt="twitter" class="social-img img-responsive"></a>
            </li>
        </ul>
    </div>
</section>


</body>

</html>
<?php
        include("footer/footer.php")
     ?>